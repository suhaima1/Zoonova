...
# llm1 = OpenAI(model_name="gpt-3.5-turbo-instruct", temperature=0.2)
...
def search_df(user_input):
    ...
    prompt = PromptTemplate(
        input_variables=["chat_history", "user_input"], template=message
    )
...
if __name__ == "__main__":
    while True:
        ...
        if user_input == "exit":
            print(memory1.chat_memory.messages, "MEMORY")
            break
        response = generate_response(user_input)
        print("------------")
        print("The output starts from here")
        print("------------")
        if "df" in response.keys():
            print("\n*********  DataFrame Results  *********\n")
            print(response["df"])
        if "gen" in response.keys():
            print("\n*********  General Info  *********\n")
            print(response["gen"])
        if "ddg" in response.keys():
            print("\n*********  DuckDuckGo Results *********\n")
            for i in range(len(response["ddg"][0])):
                print(response["ddg"][0][i])
                print(response["ddg"][1][i])
                print(response["ddg"][2][i])
